--[[
    RVX-hub: Greedy Growers Tab (ใช้ WindUI element จริง แทน GUI ลอยแบบเดิม)

    ไฟล์นี้คือ "per-game loader" — โหลด Core.lua (Hub กลาง) มาสร้าง Window/Home/Settings ก่อน
    แล้วค่อยเพิ่ม Tab "Greedy Growers" ทับเข้าไป ดังนั้นฟีเจอร์ Auto Buy/Sell พวกนี้
    จะโผล่ "เฉพาะตอนโหลดไฟล์นี้" เท่านั้น (เช่นเดียวกับ brookhaven.lua ที่โผล่เฉพาะแมพนั้น)
    ถ้าเผลอเอาไฟล์นี้ไปรันในแมพอื่น มันจะยังพยายามทำงาน แต่จะหา BigField/ConveyorSeeds
    ไม่เจอ แล้วก็แค่ไม่มีอะไรให้ซื้อ/ขายเฉยๆ (ไม่ error พัง)

    วิธีใช้จริง: แก้ CORE_URL ด้านล่างให้เป็น raw.githubusercontent.com ของ Core.lua
...
-- ===========================================================
-- ตั้งแต่บรรทัดนี้ลงไปคือของเฉพาะแมพ Greedy Growers เท่านั้น
-- ===========================================================
...
print("[Greedy Growers] โหลด Tab เรียบร้อย — ฟีเจอร์นี้จะโผล่เฉพาะตอนรันไฟล์นี้เท่านั้น")
