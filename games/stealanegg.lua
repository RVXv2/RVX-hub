-- RVX Hub module adapter for Steal An Egg
-- Masmo game logic preserved; custom Masmo/NAP UI replaced by RVX UI.
local Module = {}
...
local Z= "MasmoHub_FlightSpeed.txt"
local z= "MasmoHub_EggSelectConfig.json"
...
if typeof(hookmetamethod)== "function" and not _G._MasmoAntiRagdollHooked then
    _G._MasmoAntiRagdollHooked = true
...
local function enableMasmoGodmode()
    b4(true)
end
local function disableMasmoGodmode()
    b4(false)
end
        ...
        if not _MasmoFpsSnap then
            ...
            pcall(function()
                if settings and settings().Rendering then
                    _MasmoFpsSnap.Quality = settings().Rendering.QualityLevel
                end
            end)
        end
...
local function Ik(...)
    ...
    pcall(function()
        local lighting = game:GetService("Lighting")
        if _MasmoFpsSnap then
            lighting.GlobalShadows = _MasmoFpsSnap.GlobalShadows
            lighting.FogEnd = _MasmoFpsSnap.FogEnd
            lighting.Brightness = _MasmoFpsSnap.Brightness
            lighting.ClockTime = _MasmoFpsSnap.ClockTime
            lighting.OutdoorAmbient = _MasmoFpsSnap.OutdoorAmbient
            pcall(function()
                if settings and settings().Rendering and _MasmoFpsSnap.Quality ~= nil then
                    settings().Rendering.QualityLevel = _MasmoFpsSnap.Quality
                end
            end)
        else
...
-- ============================================================
-- ADDED FEATURES: Egg ESP (HUD), Server Hop, Webhook (Masmo style)
...
local function MasmoEnsureEspFolder()
	if MasmoESP.folder and MasmoESP.folder.Parent then
		return MasmoESP.folder
	end
...
local function MasmoEspClear()
	for uid, entry in pairs(MasmoESP.cache) do
		pcall(function()
			if entry.highlight then entry.highlight:Destroy() end
			if entry.billboard then entry.billboard:Destroy() end
			if entry.arrow then entry.arrow:Destroy() end
		end)
		MasmoESP.cache[uid] = nil
	end
	if MasmoESP.folder then
		pcall(function() MasmoESP.folder:ClearAllChildren() end)
	end
end

local function MasmoRarityColor(rarity)
...
local function MasmoEspAttach(uid, worldPos, rarity, areaName)
	if not worldPos then return end
	local folder = MasmoEnsureEspFolder()
	local entry = MasmoESP.cache[uid]
	local color, rank = MasmoRarityColor(rarity)
...
local function MasmoEggPos(rec)
...
local function MasmoEspScan()
	if not h or not h.eggESP then
		return
	end
	local now = os.clock()
	if now - (MasmoESP.lastScan or 0) < 1.0 then
		return
	end
	MasmoESP.lastScan = now
			...
			if type(rec) == "table" then
				...
				if uid and state ~= "Carried" and state ~= 2 then
					...
					if pos then
						...
						MasmoEspAttach(tostring(uid), pos, rarity, area)
			...
			if yname and yname ~= "" and not string.find(tostring(yname), "FirstArea") then
				...
				if ok and pivot and pivot.Position.X >= 530 then
					...
					if not seen[uid] then
						...
						MasmoEspAttach(uid, pivot.Position, rarity, area)
						seen[uid] = true
					end
				end
			end
...
local function MasmoEspStart()
	if MasmoESP.conn then return end
	MasmoEnsureEspFolder()
	-- Background loop (does not compete with Auto Steal Heartbeat movement)
	MasmoESP.conn = true
	task.spawn(function()
		while MasmoESP.conn and h and h.alive do
			if h.eggESP then
				pcall(MasmoEspScan)
			end
			task.wait(1.0)
	...
	task.defer(function() pcall(MasmoEspScan) end)
end

local function MasmoEspStop()
	MasmoESP.conn = nil
	MasmoEspClear()
end

-- Webhook — Masmo monochrome embed style (not the usual colorful bot style)
local function MasmoWebhook(title, description, fields)
	...
	local embed = {
		title = "Masmo · " .. tostring(title or "Event"),
		description = tostring(description or ""),
		color = 0xE8E8E8, -- near-white accent
		footer = { text = "Masmo HUB · Steal an Egg" },
...
local function MasmoHopNow()
	...
	if h.webhookOnHop then
		MasmoWebhook("Server Hop", "Teleporting to a new public server.", {
	...
	while true do
		...
		elseif h.autoHop then
			if (h.hopCount or 0) >= (h.maxHops or 15) then
				h.autoHop = false
				MasmoWebhook("Auto Hop", "Max hops reached — stopped.")
			else
...
local function rM(e,r,y,...) pcall(function(...)
        ...
        if u and u:IsA( "GuiObject" )then
            ...
            local j=u:FindFirstChild( "MasmoAccentStroke" )or u:FindFirstChildOfClass( "UIStroke" )
            if j then
                j:Destroy()
            end
...
local function aM(...) h.alive = false pcall(MasmoEspStop)pcall(Ik)pcall(Ak)pcall(function(...) y:Set3dRenderingEnabled( true )
    end
    )pcall(function(...)
        local y=r:FindFirstChild( "MasmoHub_EggESP" )
        if y then
            y:Destroy()
        end
    end
    ...
    pcall(function(...)
        for r,y in ipairs(game.CoreGui :GetChildren())do
            if y.Name :find( "Masmo_" )or y.Name :find( "MasmoHubUI" )or y.Name :find( "WindUI" )then
                y:Destroy()
            end
        end
    end
...
local function oM(...)
    -- RVX Hub adapter: Masmo game logic stays intact; only the UI layer is replaced.
    h.gui = nil
    local LocalPlayerRef = o
    local TeleportServiceRef = TeleportService

    local function Notify(title, content, duration)
        pcall(function()
            WindUI:Notify({
                Title = tostring(title or "Steal An Egg"),
    ...
    FarmTab:Toggle({
        Title = "Auto Steal",
        Desc = "ขโมยไข่อัตโนมัติด้วยโหมด Tween",
    ...
    FarmTab:Toggle({
        Title = "Auto Place",
        Desc = "วางไข่ที่ฐานอัตโนมัติทุก 5 ครั้ง",
        ...
        Callback = function(state)
            ...
            if state then
                task.spawn(function()
                    while h.alive and (h.autoPlace or h.autoPlaceEvery5) do
                        local ok, cnt = pcall(y4)
                        if ok and cnt and cnt > 0 and not h.isBatchPlacing and not h.glidingToTarget and not h.securingEgg and not h.teleporting then
                            pcall(function()
            ...
            Notify("Auto Place", state and "เปิดแล้ว" or "ปิดแล้ว")
        end,
    })

    FarmTab:Toggle({
        Title = "Auto Hatch",
        Desc = "ฟักไข่ที่พร้อมอัตโนมัติ",
        Value = h.autoHatch == true,
        Callback = function(state)
            h.autoHatch = state
            Notify("Auto Hatch", state and "เปิดแล้ว" or "ปิดแล้ว")
        end,
    })

    FarmTab:Toggle({
        Title = "Auto Return",
        Desc = "กลับฐานหลังขโมยไข่สำเร็จ",
        Value = h.autoGlide == true,
        Callback = function(state)
            h.autoGlide = state
            Notify("Auto Return", state and "เปิดแล้ว" or "ปิดแล้ว")
        end,
    })

    FarmTab:Toggle({
        Title = "Auto Treadmill",
        Desc = "วิ่งลู่วิ่งอัตโนมัติเมื่อไม่มีเป้าหมาย",
    ...
    FarmTab:Toggle({
        ...
        Desc = "อัปเกรดลู่วิ่งเมื่อมีเงินพอ",
        Value = h.autoUpgradeTreadmill == true,
        Callback = function(state)
    ...
    FarmTab:Toggle({
        ...
        Desc = "ซื้อและใส่ Trail ที่ดีที่สุดอัตโนมัติ",
    ...
    FarmTab:Section({ Title = "คำสั่งทันที", Desc = "สั่งทำงานด้วยตัวเอง" })

    FarmTab:Button({
        Title = "Place Egg",
        Desc = "นำไข่ในกระเป๋าไปวางที่ฐานทันที",
        Callback = function()
            h.statusText = "[Manual] Depositing eggs..."
            pcall(g4, h.glideSpeed)
            pcall(v4)
            pcall(u4)
            h.isReturning = false
            h.delivering = false
    ...
    FarmTab:Button({
        Title = "Hatch Once",
        Desc = "ฟักไข่ที่พร้อมทันที",
        Callback = function()
            pcall(v4)
        end,
    })

    CharacterTab:Section({ Title = "การเคลื่อนไหว", Desc = "ปรับการเคลื่อนที่ของระบบ Masmo" })
    CharacterTab:Slider({
        Title = "Glide Speed",
        Desc = "ความเร็วบิน/เคลื่อนที่ไปหาไข่",
    ...
    CharacterTab:Section({ Title = "ความปลอดภัย" })
    CharacterTab:Toggle({
        Title = "Godmode",
        Desc = "ป้องกันความเสียหายและกับดัก",
        Value = h.godmode == true,
        Callback = function(state)
            if state then
                pcall(enableMasmoGodmode)
            else
                pcall(disableMasmoGodmode)
            end
        end,
    })
    CharacterTab:Toggle({
        Title = "Anti Trap",
        Desc = "หลบ/ลดผลกระทบจากกับดักระหว่างเคลื่อนที่",
    ...
    CharacterTab:Section({ Title = "แก้ปัญหา" })
    CharacterTab:Button({
        Title = "Get Unstuck",
        Desc = "แก้ปัญหาติดค้างและคืนสถานะตัวละคร",
        Callback = function()
            pcall(M4)
            pcall(C4)
            pcall(D4)
    ...
    CharacterTab:Button({
        ...
        Desc = "ล้างความเร็ว/แรงผลักและสถานะติดค้าง",
        Callback = function()
            pcall(D4)
            pcall(u4)
        end,
    ...
    TargetTab:Section({ Title = "เป้าหมาย", Desc = "เลือกประเภทไข่ที่ระบบจะให้ความสำคัญ" })
    TargetTab:Toggle({
        Title = "Auto Steal Secret+",
        Desc = "เก็บ Secret ขึ้นไปเสมอ",
    ...
    for _, name in ipairs(M or {}) do
        table.insert(zoneValues, name)
        if h.selectedZones and h.selectedZones[name] then
            table.insert(zoneDefault, name)
        end
    end
    TargetTab:Dropdown({
        Title = "Zones",
        Desc = "เลือกหลายโซนที่ต้องการขโมย",
        Values = zoneValues,
        Value = zoneDefault,
        Multi = true,
        AllowNone = true,
    ...
    local rarityValues = {}
    local rarityDefault = {}
    for _, name in ipairs(X or {}) do
        table.insert(rarityValues, name)
        if h.selectedRarities and h.selectedRarities[name] then
            table.insert(rarityDefault, name)
        end
    end
    TargetTab:Dropdown({
        Title = "Rarities",
        Desc = "เลือกระดับความหายากหลายรายการ",
        Values = rarityValues,
        Value = rarityDefault,
        Multi = true,
        AllowNone = true,
    ...
    SystemTab:Section({ Title = "ทั่วไป" })
    SystemTab:Toggle({
        Title = "Anti AFK",
        Desc = "ป้องกันการเตะออกเพราะไม่ขยับ",
        Value = h.antiAFK == true,
        Callback = function(state)
            h.antiAFK = state
            pcall(x)
            if state then pcall(bk) else pcall(Ak) end
        end,
    })
    SystemTab:Toggle({
        Title = "Egg ESP",
        Desc = "แสดงตำแหน่งไข่บนหน้าจอ",
        Value = h.eggESP == true,
        Callback = function(state)
            h.eggESP = state
            if state then pcall(MasmoEspStart) else pcall(MasmoEspStop) end
        end,
    })
    SystemTab:Toggle({
        Title = "Auto Index",
        Desc = "ลองเก็บ Index / Codex อัตโนมัติ",
        Value = h.autoIndex == true,
        Callback = function(state)
            h.autoIndex = state
            if state then task.spawn(MasmoTryIndex) end
        end,
    })
    SystemTab:Toggle({
        Title = "FPS Boost",
        Desc = "ลดเอฟเฟกต์กราฟิกเพื่อช่วยเฟรมเรต",
        Value = h.performanceMode == true,
        Callback = function(state)
            h.performanceMode = state
            if state then pcall(Mk) else pcall(Ik) end
        end,
    })

    SystemTab:Section({ Title = "Server Hop" })
    SystemTab:Toggle({
        Title = "Auto Hop",
        Desc = "สลับเซิร์ฟเวอร์อัตโนมัติ",
        Value = h.autoHop == true,
        Callback = function(state)
            h.autoHop = state
        end,
    })
    SystemTab:Slider({
        Title = "Max Hops",
        Desc = "จำนวนครั้งสูงสุดก่อนหยุด",
        Value = { Min = 1, Max = 100, Default = h.maxHops or 15 },
        Callback = function(v) h.maxHops = v end,
    })
    SystemTab:Slider({
        Title = "Hop Delay",
        Desc = "หน่วงเวลาก่อน Hop แต่ละครั้ง (วินาที)",
        Value = { Min = 5, Max = 120, Default = h.hopDelay or 20 },
        Callback = function(v) h.hopDelay = v end,
    })
    SystemTab:Button({
        Title = "Hop Now",
        Desc = "ย้ายไป public server อื่นทันที",
        Callback = function() pcall(MasmoHopNow) end,
    })
    SystemTab:Button({
        Title = "Reset Hop Count",
        Desc = "รีเซ็ตตัวนับ Hop",
        Callback = function() h.hopCount = 0 end,
    })

    SystemTab:Section({ Title = "Webhook" })
    SystemTab:Toggle({
        Title = "Webhook Alerts",
        Desc = "เปิดแจ้งเตือนผ่าน Discord Webhook",
        Value = h.webhookEnabled == true,
        Callback = function(state)
            h.webhookEnabled = state
            if state and (h.webhookUrl or "") ~= "" then
                pcall(MasmoWebhook, "Online", "Webhook linked to RVX Hub.")
            end
        end,
    })
    SystemTab:Input({
        Title = "Webhook URL",
        Desc = "ใส่ Discord webhook URL",
        Value = h.webhookUrl or "",
        Placeholder = "https://discord.com/api/webhooks/...",
        Callback = function(text) h.webhookUrl = tostring(text or "") end,
    })
    SystemTab:Toggle({
        Title = "Alert on Place",
        Desc = "แจ้งเตือนเมื่อวางไข่",
        Value = h.webhookOnSteal ~= false,
        Callback = function(state) h.webhookOnSteal = state end,
    })
    SystemTab:Toggle({
        Title = "Alert on Hop",
        Desc = "แจ้งเตือนเมื่อ Hop",
        Value = h.webhookOnHop ~= false,
        Callback = function(state) h.webhookOnHop = state end,
    })
    SystemTab:Button({
        Title = "Test Webhook",
        Desc = "ส่งข้อความทดสอบ",
        Callback = function()
            h.webhookEnabled = true
            pcall(MasmoWebhook, "Test", "RVX Hub webhook is working!")
        end,
    })

    SystemTab:Section({ Title = "อื่นๆ" })
    SystemTab:Button({
        Title = "Rejoin Server",
        Desc = "เข้าเซิร์ฟเวอร์เดิมใหม่",
        Callback = function()
            pcall(function()
                TeleportServiceRef:TeleportToPlaceInstance(game.PlaceId, game.JobId, LocalPlayerRef)
            end)
        end,
    })
    SystemTab:Button({
    ...
    _G.NAP_OnEggStolen = function(data)
        pcall(function()
            local name = data and (data.Category or data.Name) or "Egg"
            local area = data and data.Area or "Field"
            Notify("Egg Stolen", tostring(name) .. " • " .. tostring(area), 2)
        end)
    end

    Notify("RVX Hub", "Steal An Egg module loaded", 2)
end

H( "[+] Initializing RVX Steal An Egg module..." )oM()task.spawn (function(...) task.wait ( 0.5 )A4()b4( true )C4()
    if o.Character then
        z4(o.Character )
    end
